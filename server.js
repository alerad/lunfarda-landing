...
app.prepare().then(() => {
    createServer((req, res) => {
        ...
        if (req.method === 'POST') {
            if (pathname === "/mailchimp") {
                mailchimp.setConfig({
                    apiKey: "e6198e1f4e525d0d5a5e112c46a5dfd3-us10",
                    server: "https://us10.api.mailchimp.com/3.0/lists/b6f47f2641",
                });
                ...
                req.on('end', function() {
                    run(JSON.parse(body.replace("null","")))
                        .then(x => console.log("MC Res"))
